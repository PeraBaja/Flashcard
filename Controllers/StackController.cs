...
class StackController(string dbConnection)
{
    ...
    public void Delete(string name)
    {
        ...
        ulong stackId = GetStackIdWith(name);
        _connection.Open();
        _connection.Execute("DELETE FROM flashcards WHERE stackId = $StackId", new { StackId = stackId });
    ...
    public void Modify(string stackNameToModify, string? newName)
    {
        ...
        _connection.Open();
        _connection.Execute("UPDATE stacks SET name = $NewName WHERE name = $Name", new { NewName = newName ?? stackNameToModify, Name = stackNameToModify });
        _connection.Close();
    }
